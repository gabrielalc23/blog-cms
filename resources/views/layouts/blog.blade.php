...
<body class="bg-white font-family-karla">
    ...
    @endif
    <!-- Top Bar Nav -->
    <nav class="w-full py-4 bg-black shadow-lg">
            ...
            <nav>
                <ul class="flex items-center justify-between font-bold text-sm text-white uppercase no-underline">
                    @foreach ($pages_nav as $page)
                        <li>
                            <a class="hover:text-gray-300 hover:underline px-4 transition duration-300" href="{{ route('page.show', $page->slug) }}">
                                {{ $page->name }}
                            </a>
                        </li>
                    @endforeach
            ...
            <div class="ml-auto flex items-center">
                @auth
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button class="py-2 px-4 bg-red-600 hover:bg-red-800 text-white font-semibold rounded-lg transition duration-300">
                            <i class="fas fa-sign-out-alt"></i>
                        </button>
                    </form>
                @else
                    <li class="inline-block">
                        <a class="py-2 px-4 mr-2 bg-gray-600 hover:bg-gray-800 text-white font-semibold rounded-lg transition duration-300" href="{{ route('register') }}">
                            <i class="fas fa-user-plus"></i>
                        </a>
                    </li>
                    <li class="inline-block">
                        <a class="py-2 px-4 bg-green-600 hover:bg-green-800 text-white font-semibold rounded-lg transition duration-300 " href="{{ route('login') }}">
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css" integrity="sha384-DyZvLx2P9l4jKtCT+8+H7h6dYbZ7V3eF1WVXv43ZQfx4EzUHlG3I8j6VUtFZs/5" crossorigin="anonymous">
